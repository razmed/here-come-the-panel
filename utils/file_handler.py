...
class FileHandler:
    ...
    def download_file(self, source_path: str, suggested_filename: str) -> bool:
        """
        Télécharger un fichier en demandant à l'utilisateur où le sauvegarder
      
        Args:
            source_path: Chemin source du fichier à télécharger
            suggested_filename: Nom suggéré pour le fichier de destination
        ...
        try:
            if not os.path.exists(source_path):
                print(f"❌ Fichier source introuvable pour téléchargement: {source_path}")
            ...
            if not save_path:
                print("⚠️ Téléchargement annulé par l'utilisateur")
                return False
          
            # Copier le fichier vers l'emplacement choisi
            shutil.copy2(source_path, save_path)
          
            print(f"✅ Fichier téléchargé: {source_path} -> {save_path}")
            messagebox.showinfo("Succès", f"✅ Fichier téléchargé avec succès !\n\nEmplacement : {save_path}")
          
            return True
          
        except Exception as e:
            print(f"❌ Erreur lors du téléchargement: {e}")
            messagebox.showerror("Erreur", f"❌ Impossible de télécharger le fichier:\n{e}")
            return False
